...
const DosageCalculator = () => {
    ...
    const [hastalikId, setHastalikId] = useState('');  // New state for hastalik_id
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.get('http://127.0.0.1:8000/api/appname/hastalikazalankilodoz/get-hastalik-azalan-doz-kilo/', {
                params: {
                    kilo: kilo,
                    ilac_id: ilacId,
                    hastalik_id: hastalikId,  // Include hastalik_id in the API request
                }
            });
    ...
    return (
        <div>
            <form onSubmit={handleSubmit}>
                ...
                <div>
                    <label htmlFor="hastalik_id">Hastalık ID:</label> {/* New input for hastalik_id */}
                    <input
                        type="number"
                        id="hastalik_id"
                        value={hastalikId}
